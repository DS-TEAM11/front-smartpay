...
const Order = ({}) => {
  ...
  const testsave = 300;
  const testtype = "적립";
  const CardImg = "https://d1c5n4ri2guedi.cloudfront.net/card/13/card_img/28201/13card.png";
  const CardCo = "신한";
  const CardLastNo = 1234;
  const CardName = "신한카드 Mr.Life";
  const CardNick = "CardNick"

  const isAIRecommended = false; // AI 추천 여부를 나타내는 변수
  let FirstMessage;
  let SecondMessage;
  let ThirdMessage;
    ...
    <section className="position-relative overflow-hidden pb-0 pt-xl-9">
      <div className="container pt-4 pt-sm-5">
        <div className="Order">
          ...
          <h4>{SecondMessage}</h4>
          <h4>{ThirdMessage}</h4>
          <div className="card-container d-flex flex-column align-items-center mt-4">
            <img src={CardImg} alt="CardImg" className="img-fluid " />
            <div className="card-info-container">
              <span>{CardNick}</span>
              <span>{CardCo} ({CardLastNo})</span>
            </div>
          </div>

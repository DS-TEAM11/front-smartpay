        ...
        const mystaticsApi = () => {
            axios
                ...
                .then((res) => {
                    ...
                    const thisMonthTotalValue =
                        responseData[firstObjectKey].thisMonthTotal;
                    const lastMonthTotalValue =
                        responseData[firstObjectKey].lastMonthTotal;
                    ...
                    const calculatedRatios = Object.entries(responseData).map(
                        ([key, item]) => {
                            const ratio = Math.round(
                                (item.thisMonth / thisMonthTotalValue) * 100,
                            );
                            return {
    ...
    const transformDataForStackedBar = () => {
        return {
            ...
            ...ratios.reduce((acc, entry) => {
                ...
                const franchiseRatio = Math.round(
                    (entry.thisMonth / lastMonthTotal) * 100,
                ); // lastMonthTotal 기준으로 비율 변환
                return {
                    ...acc,
                            ...
                            key !== 'lastMonthTotal' && (
                                <div
                                    ...
                                    style={{
                                        ...
                                        backgroundColor:
                                            franchiseData[
                                                Object.keys(franchiseData).find(
                                                    (code) =>
                                                        franchiseData[code]
                                                            .name === key,
                                                )
